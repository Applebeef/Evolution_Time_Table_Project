...
public class TimeTableSolutionDisplayer {
    ...
    Map<Integer, Row> integerRowMap = new HashMap<>(); //First key stands for hours, second key stands for days.
    ...
    public TimeTableSolutionDisplayer(TimeTableSolution solution, ResultDisplay resultDisplay) {
        ...
        for (int i = 1; i <= solution.getTimeTable().getHours(); i++) {
            int finalI = i;
            if (!integerRowMap.containsKey(i)) {
                integerRowMap.put(i, new Row(i, resultDisplay, filteredSolutionList.stream().filter(fifth -> fifth.getHour().equals(finalI)).collect(Collectors.toList())));
            }
        }
    }

    public Row getDisplay(int hour) {
        return integerRowMap.getOrDefault(hour, null);
    }
}
